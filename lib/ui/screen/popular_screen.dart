...
import 'package:goldenmovie/bloc/popular_bloc.dart';
import 'package:goldenmovie/state/popular_state.dart';
import 'package:goldenmovie/ui/screen/movie_detail.dart';
import 'package:intl/intl.dart';

class PopuplarScreen extends StatefulWidget {
  const PopuplarScreen({Key? key}) : super(key: key);

  @override
  State<PopuplarScreen> createState() => _PopuplarScreenState();
}

class _PopuplarScreenState extends State<PopuplarScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: [
          Expanded(
            child: StreamBuilder<PopularState>(
              stream: popularBloc.popularState,
              builder: (context, snapshot) {
                PopularState? state = snapshot.data;
                                    ...
                                    builder: (context) => MovieDetailPage(
                                        movieId:
                                            state.data?.results?[index].id ??
                                                0,movie: state.data?.results?[index].genre,),
                                  ));
                            },
                            child: Card(
                              child: Row(
